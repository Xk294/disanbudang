    ...
    <div class="max-w-2xl mx-auto px-4 py-12">
      ...
      <div v-else-if="!user" class="text-center py-16">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
          <button
            id="me-google-signin"
            class="inline-flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-white text-stone-800 font-semibold text-sm hover:bg-stone-100 transition-colors disabled:opacity-50 w-full sm:w-auto cursor-pointer"
            :disabled="signingInProvider !== null"
            @click="handleSignIn('google')"
          >
            <Icon v-if="signingInProvider === 'google'" name="mdi:loading" class="w-5 h-5 animate-spin" />
            ...
            {{ signingInProvider === 'google' ? 'Đang đăng nhập…' : 'Đăng nhập với Google' }}
          </button>

          <button
            id="me-facebook-signin"
            class="inline-flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-[#1877F2] hover:bg-[#166fe5] text-white font-semibold text-sm transition-colors disabled:opacity-50 w-full sm:w-auto cursor-pointer"
            :disabled="signingInProvider !== null"
            @click="handleSignIn('facebook')"
          >
            <Icon v-if="signingInProvider === 'facebook'" name="mdi:loading" class="w-5 h-5 animate-spin text-white" />
            <Icon v-else name="mdi:facebook" class="w-5 h-5 text-white" />
            {{ signingInProvider === 'facebook' ? 'Đang đăng nhập…' : 'Đăng nhập với Facebook' }}
          </button>
        </div>
...
const { user, authReady, isAdmin, initAuthListener, signInWithGoogle, signInWithFacebook, signOut, syncUserToDb } = useAuth()
